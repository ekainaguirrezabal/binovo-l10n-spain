...
class AccountInvoice(models.Model):
    ...
    lroe_response_line_ids = fields.Many2many(
        comodel_name='lroe.operation.response.line', compute='_compute_lroe_response_line_ids',
        string='Responses')

    @api.depends('tbai_invoice_ids', 'tbai_invoice_ids.state', 'tbai_cancellation_ids', 'tbai_cancellation_ids.state')
    def _compute_lroe_response_line_ids(self):
        for record in self:
            ...
            response_lines = response_line_model.search([('tbai_response_id', 'in', response_ids)])
