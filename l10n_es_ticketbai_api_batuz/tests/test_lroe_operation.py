...
from ..models.lroe_operation_response import LROEOperationResponseState, LROEOperationResponseLineState
...
TEST_01_XSD_SCHEMA = 'Test-LROE_PJ_240_1_1_FacturasEmitidas_ConSG_AltaPeticion_V1_0_2.xsd'
TEST_02_XSD_SCHEMA = 'Test-LROE_PF_140_1_1_Ingresos_ConfacturaConSG_AltaPeticion_V1_0_2.xsd'
TEST_03_XSD_SCHEMA = 'Test-LROE_PJ_240_1_1_FacturasEmitidas_ConSG_AnulacionPeticion_V1_0_0.xsd'
TEST_04_XSD_SCHEMA = 'Test-LROE_PF_140_1_1_Ingresos_ConfacturaConSG_AnulacionPeticion_V1_0_0.xsd'
TEST_EXEC_PATH = os.path.dirname(os.path.abspath(__file__))
TEST_RESPONSE_DIR = os.path.join(TEST_EXEC_PATH, 'lroe_response_files')
TEST_05_RESPONSE_GZ_FILENAME = os.path.join(TEST_RESPONSE_DIR,
                                            'Ejemplo_1_LROE_PJ_240_FacturasEmitidasConSG_B00000034_Correcta_Resp.gz')
TEST_06_RESPONSE_GZ_FILENAME = os.path.join(TEST_RESPONSE_DIR,
                                            'Ejemplo_1_LROE_PF_140_IngresosConFacturaConSG_79732487C_Correcta_Resp.gz')
TEST_07_RESPONSE_GZ_FILENAME = \
    os.path.join(TEST_RESPONSE_DIR,
                 'Ejemplo_1_LROE_PJ_240_FacturasEmitidasConSG_B00000034_Parc_Correcta_Resp.gz')
TEST_08_RESPONSE_GZ_FILENAME = \
    os.path.join(TEST_RESPONSE_DIR,
                 'Ejemplo_1_LROE_PF_140_IngresosConFacturaConSG_79732487C_Parc_Correcta_Resp.gz')
TEST_09_RESPONSE_GZ_FILENAME = os.path.join(TEST_RESPONSE_DIR,
                                            'Ejemplo_1_LROE_PJ_240_FacturasEmitidasConSG_B00000034_Incorrecta_Resp.gz')
TEST_10_RESPONSE_GZ_FILENAME = \
    os.path.join(TEST_RESPONSE_DIR,
...
class TestL10nEsTicketBAIAPIBatuz(TestL10nEsTicketBAIAPI):
    ...
    def create_tbai_cancel_invoice(self, str_number):
        ...
        invoice = self.create_tbai_national_invoice_cancellation(name='TBAITEST/' + str_number,
    ...
    def create_tbai_invoice(self, str_number):
        uid = self.tech_user.id
        invoice = self.create_tbai_national_invoice(name='TBAITEST/' + str_number,
    ...
    def setUp(self):
        ...
        self.test_01_schema_doc = etree.parse(test_01_xsd_filepath, parser=etree.ETCompatXMLParser())
        test_02_xsd_filepath = os.path.abspath(
            os.path.join(schemas_dirpath,
                         '%s/%s' % (schemas_version_dirname, TEST_02_XSD_SCHEMA)))
        self.test_02_schema_doc = etree.parse(test_02_xsd_filepath, parser=etree.ETCompatXMLParser())
        test_03_xsd_filepath = os.path.abspath(
            os.path.join(schemas_dirpath,
                         '%s/%s' % (schemas_version_dirname, TEST_03_XSD_SCHEMA)))
        self.test_03_schema_doc = etree.parse(test_03_xsd_filepath, parser=etree.ETCompatXMLParser())
        test_04_xsd_filepath = os.path.abspath(
            os.path.join(schemas_dirpath,
                         '%s/%s' % (schemas_version_dirname, TEST_04_XSD_SCHEMA)))
        self.test_04_schema_doc = etree.parse(test_04_xsd_filepath, parser=etree.ETCompatXMLParser())
    ...
    def test_03_xml_cancel_model_pj_240(self):
        ...
        tbai_cancel_invoice_ids = tbai_invoice1_cancel_ids.ids + tbai_invoice2_cancel_ids.ids
        self.main_company.lroe_model = LROEModelEnum.model_pj_240.value
        ...
        lroe_op_cancel_model_pj_240 = self.lroe_op_model.create(lroe_op_cancel_pj_240_dict)
    ...
    def test_04_xml_cancel_model_pf_140(self):
        ...
        lroe_op_cancel_model_pf_140 = self.lroe_op_model.create(lroe_op_cancel_pf_140_dict)
    ...
    def test_05_prepare_lroe_response_values_alta_model_pj_240(self):
        ...
        with open(TEST_05_RESPONSE_GZ_FILENAME, 'rb') as response_data_file:
            lroe_srv_response = lroe_api.LROEOperationResponse(data=response_data_file.read(), headers=response_headers)
            lroe_response_values = \
                self.env['lroe.operation.response'].prepare_lroe_response_values(lroe_srv_response,
                                                                                 lroe_op_alta_model_pj_240)
        self.assertEqual(lroe_response_values.get('lroe_operation_id'), lroe_op_alta_model_pj_240.id)
        self.assertEqual(lroe_response_values.get('state'), LROEOperationResponseState.CORRECT.value)
        ...
        for response_line_id in lroe_response_values.get('response_line_ids'):
            self.assertTrue(response_line_id[2].get('state'), LROEOperationResponseLineState.CORRECT.value)
    ...
    def test_06_prepare_lroe_response_values_alta_model_pf_140(self):
        ...
        self.assertEqual(lroe_response_values.get('lroe_operation_id'), lroe_op_alta_model_pf_140.id)
        self.assertEqual(lroe_response_values.get('state'), LROEOperationResponseState.CORRECT.value)
        self.assertTrue(len(lroe_response_values.get('xml')) > 0)
        self.assertTrue(len(lroe_response_values.get('response_line_ids')) > 0)
        for response_line_id in lroe_response_values.get('response_line_ids'):
    ...
    def test_07_prepare_lroe_response_values_alta_model_pj_240_w_errors(self):
        ...
        with open(TEST_07_RESPONSE_GZ_FILENAME, 'rb') as response_data_file:
            ...
            lroe_response_values = \
                self.env['lroe.operation.response'].prepare_lroe_response_values(lroe_srv_response,
                                                                                 lroe_op_alta_model_pj_240)
        self.assertEqual(lroe_response_values.get('lroe_operation_id'), lroe_op_alta_model_pj_240.id)
        self.assertEqual(lroe_response_values.get('state'), LROEOperationResponseState.PARTIALLY_CORRECT.value)
        self.assertTrue(len(lroe_response_values.get('xml')) > 0)
        self.assertTrue(len(lroe_response_values.get('response_line_ids')) > 0)
        response_correct = 0
        ...
        for response_line_id in lroe_response_values.get('response_line_ids'):
            ...
            if response_line_dict.get('state') == LROEOperationResponseLineState.CORRECT.value:
                response_correct += 1
            if response_line_dict.get('state') == LROEOperationResponseLineState.INCORRECT.value:
        ...
        with open(TEST_09_RESPONSE_GZ_FILENAME, 'rb') as response_data_file:
            ...
            lroe_response_values = \
                self.env['lroe.operation.response'].prepare_lroe_response_values(lroe_srv_response,
                                                                                 lroe_op_alta_model_pj_240)
        self.assertEqual(lroe_response_values.get('lroe_operation_id'), lroe_op_alta_model_pj_240.id)
        self.assertEqual(lroe_response_values.get('state'), LROEOperationResponseState.INCORRECT.value)
        self.assertTrue(len(lroe_response_values.get('xml')) > 0)
        self.assertTrue(len(lroe_response_values.get('response_line_ids')) > 0)
        for response_line_id in lroe_response_values.get('response_line_ids'):
            self.assertEqual(response_line_id[2].get('state'), LROEOperationResponseLineState.INCORRECT.value)
    ...
    def test_10_prepare_lroe_response_values_cancel_model_pf_140(self):
        ...
        self.assertEqual(lroe_response_values.get('lroe_operation_id'), lroe_op_cancel_model_pf_140.id)
        self.assertEqual(lroe_response_values.get('state'), LROEOperationResponseState.CORRECT.value)
        self.assertTrue(len(lroe_response_values.get('xml')) > 0)
        self.assertTrue(len(lroe_response_values.get('response_line_ids')) > 0)
        for response_line_id in lroe_response_values.get('response_line_ids'):
